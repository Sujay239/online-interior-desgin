  ...
  return (
    ...
    >
      <div
        ...
        className={cn("relative isolate w-full select-none overflow-hidden touch-none", className)}
...
export const ComparisonHandle = ({ className, children, ...props }: ComparisonHandleProps) => {
  ...
  return (
    <motion.div
      aria-hidden="true"
      className={cn(
        "-translate-x-1/2 absolute top-0 z-50 flex h-full w-10 items-center justify-center pointer-events-none", // pointer-events-none prevents blocking user drags
    ...
    >
      {children ?? (
        <>
          <div className="-translate-x-1/2 absolute left-1/2 h-full w-0.5 bg-white shadow-[0_0_10px_rgba(0,0,0,0.2)]" />
          {mode === "drag" && (
            <div className="z-50 flex items-center justify-center rounded-full bg-white p-2 shadow-lg border border-neutral-100">
              <GripVerticalIcon className="h-4 w-4 select-none text-neutral-400" />
            </div>
          )}
        </>
      )}
